...
public class TreeRebuilder {
	...
	public String resolveString(String string) {
		...
		if (string.startsWith(Constants.STR_DOUBLE_QUOTE) || string.startsWith(Constants.STR_SINGLE_QUOTE)) {
	...
	public Node refactor(Node root) {
		...
		if (application.isMacroOn()) {
			...
			// real refactor must run after iterated to make sure iterator
			// correct
			if (!actions.isEmpty()) {
	...
	public void nodeReplace(Node tobe, NodeList with) {
		if (tobe != null) {
			try {
				...
				Method action = NodeAction.class.getDeclaredMethod("replace", new Class[] { Node.class, NodeList.class });
	...
	public void nodeReplace(Node tobe, Node with) {
		if (tobe != null) {
			try {
				Method action = NodeAction.class.getDeclaredMethod("replace", new Class[] { Node.class, Node.class });
				Object[] msg = new Object[] { tobe, with };
				actions.add(action);
				msgs.add(msg);
			} catch (Exception e) {
	...
	public void nodeRemove(Node node) {
		if (node != null) {
			try {
				Method action = NodeAction.class.getDeclaredMethod("remove", new Class[] { Node.class });
	...
	public void nodeInsertAfter(Node node, Node toAdd) {
		if (node != null && toAdd != null) {
			try {
				Method action = NodeAction.class.getDeclaredMethod("insertAfter", new Class[] { Node.class, Node.class });
	...
	public void nodeInsertBefore(Node node, Node toAdd) {
		if (node != null && toAdd != null) {
			try {
				Method action = NodeAction.class.getDeclaredMethod("insertBefore", new Class[] { Node.class, Node.class });
				Object[] msg = new Object[] { node, toAdd };
				actions.add(action);
				msgs.add(msg);
			} catch (Exception e) {
	...
	public void nodeAddChildLast(Node parent, Node toAdd) {
		if (parent != null && toAdd != null) {
			try {
				Method action = NodeAction.class.getDeclaredMethod("addChildLast", new Class[] { Node.class, Node.class });
	...
	public void nodeAddChildFirst(Node parent, Node toAdd) {
		if (parent != null && toAdd != null) {
			try {
				Method action = NodeAction.class.getDeclaredMethod("addChildFirst", new Class[] { Node.class, Node.class });
				Object[] msg = new Object[] { parent, toAdd };
				actions.add(action);
				msgs.add(msg);
			} catch (Exception e) {
	...
	public void nodeExchange(Node node1, Node node2) {
		if (node1 != null && node2 != null) {
			try {
				Method action = NodeAction.class.getDeclaredMethod("exchange", new Class[] { Node.class, Node.class });
