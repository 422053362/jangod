...
public class NodeList implements Iterable<Node>, Serializable, Cloneable {
	...
	boolean postend(Node to, Node add) {
		if (to == null || add == null || head == null || to.parent != head.parent || add.parent == to.parent) {
			return false;
		}
	...
	boolean jumpAfter(Node to, Node jump) {
		if (to == null || jump == null || head == null || to.parent != head.parent || jump.parent != head.parent) {
			return false;
		}
		if (jump != head) {
	...
	boolean alternate(Node e, Node n) {
		if (e == null || n == null || head == null || e.parent != head.parent || n.parent != head.parent) {
			return false;
		}
	...
	boolean replace(Node to, NodeList with) {
		if (to == null || with.size == 0 || head == null || to.parent != head.parent || with.head.parent == head.parent) {
			return false;
		}
	...
	boolean replace(Node to, Node with) {
		if (to == null || with == null || head == null || to.parent != head.parent || with.parent == head.parent) {
			return false;
		}
	...
	boolean remove(Node e) {
		if (e == null || head == null || e.parent != head.parent) {
			return false;
		}
		if (e == head) {
			head = head.successor;
