...
public class MessageTag implements Tag {
	...
	public String interpreter(NodeList carries, String helpers, JangodInterpreter interpreter) throws InterpretException {
		...
		if (tokens.length < 1) {
			throw new InterpretException("Tag 'message' expects at least 1 helper >>> " + tokens.length);
		} else {
		...
		String message = key;
		MessageSource messageSource = interpreter.getApplication().getMessageSource();
		if (messageSource != null) {
			message = messageSource.getMessage(key, args, interpreter.getLocale());
		}

		return message;
	}

	private String resolveToken(String token, JangodInterpreter interpreter) throws InterpretException {
