...
public class ExtendsMacro implements Macro {
	...
	public void refactor(Node current, String helpers, TreeRebuilder rebuilder) throws ParseException {
		...
		try {
			String fullName = ResourceManager.getFullName(templateFile, rebuilder.getWorkspace(), rebuilder.getConfiguration().getWorkspace());
			// TODO STOP LOOP EXTENDS
			Node extendsRoot = TreeParser.parser(new TokenParser(ResourceManager.getResource(fullName, rebuilder.getConfiguration().getEncoding())));
