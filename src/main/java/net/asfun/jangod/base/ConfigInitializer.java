			...
			try {
				...
				if (properties.containsKey("language")) {
					try {
						...
						JangodLogger.warning("Language value from config file is invalid(use default) >>> " + properties.getProperty("language"));
					}
				} else {
					config.setLocale(Locale.CHINESE);
				}
				if (properties.containsKey("timezone")) {
					try {
						...
						JangodLogger.warning("Timezone value from config file is invalid(use default) >>> " + properties.getProperty("timezone"));
					}
				} else {
					config.setTimezone(TimeZone.getDefault());
				}
